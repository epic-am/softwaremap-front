...
var ServiceGlobalValue = React.createClass({
  ...
  render: function() {
    ...
    var tooltipValue;
    if (this.props.tooltipRenderCallback) {
      tooltipValue = this.props.tooltipRenderCallback.call(this, this.props.data.value);
    } else if (this.props.data && this.props.data.value) {
      tooltipValue = this.props.data.value;
    } else {
      value = Constants.NO_VALUE;
    }

    var status = this.props.data.status ? this.props.data.status : Constants.KO_STATUS;
    var finalClassName = "elem nav nav-pills nav-pills-" + Constants.statusClassMap[status];
    ...
    return (
      <ul className={finalClassName} role="tablist" data-toggle="tooltip" data-placement="top" title={tooltipValue}>
        <li className="active">
